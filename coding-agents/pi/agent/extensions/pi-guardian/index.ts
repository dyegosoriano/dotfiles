...
import { assertPathAccessAllowed, BLOCKED_PATHS, PathGuardError } from "./src/guard-path.js";
...
import { assertFileAccessAllowed, FileGuardError } from "./src/guard-file.js";
    ...
    for (const { path, operation } of paths) {
      try {
        ...
        if (gitignoreGuardEnabled) {
          const gitignoreResult = guardGitignoreAccess({ path, rootDir: process.cwd(), operation: operation as GitignoreGuardOperation, });

          if (!gitignoreResult.allowed) return { block: true, reason: gitignoreResult.message };
        }
      } catch (error) {
        if (error instanceof FileGuardError || error instanceof PathGuardError || error instanceof GitignoreGuardError) return { block: true, reason: error.message };
